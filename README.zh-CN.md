...
2. 选择 **节点** 或 **整段路段**。
3. 在地图上左键单击节点或路段以选中目标；右键用于取消目标选择。
4. 搜索资产列表，并通过圆形控件将资产标记为禁行。
5. 点击“应用到所选目标”。禁行清单为空时，表示放行全部兼容资产。

被选中的条目表示将被**禁止通行**的资产。“全部资产禁行”和“全部资产放行”作用于完整资产目录，不受当前筛选条件或页码影响。已识别的牵引车辆组在折叠状态下会同时修改列出的车厢，展开后可逐项修改。内容发生变化后可在面板内刷新资产目录。`Ctrl+Shift+X` 可作为可选的工具开关快捷键。
