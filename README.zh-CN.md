...
- 悬浮资产可查看以 km/h 显示的最高速度、加速度和制动减速度。
...
被选中的条目表示将被**禁止通行**的资产。“全部资产禁行”和“全部资产放行”只修改当前所选目标的待应用清单；只有点击“应用到所选目标”后才会写入地图。切换目标时会载入该目标自己保存的清单。内容发生变化后可在面板内刷新资产目录。`Ctrl+Shift+X` 可作为可选的工具开关快捷键。
